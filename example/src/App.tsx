...
import { GenericEscposPrint, BluetoothTscPrinter, BluetoothEscposPrinter } from 'react-native-generic-escpos-print';
...
export default function App() {
  ...
  async function getDevices() {
    let pareados: String[] = await GenericEscposPrint.enableBluetooth();
    setDevices(pareados.map((de) => {
      return JSON.parse(String(de));
    }));
  }
   ...
   */
    if (await GenericEscposPrint.isDeviceConnected()) {
      console.log("Dispositivo conectado !");
    } else {
      let printerName = await GenericEscposPrint.connect(address);
      setPrinter(printerName);
      console.log(printerName);
    }
  }

  async function printTSC(text: String) {
    await BluetoothEscposPrinter.printerInit();
    await BluetoothEscposPrinter.verticalPrint(); 
    ...
    await BluetoothTscPrinter.printLabel(
      {
        ...
        height: 30,
        gap: 20,
        direction: BluetoothTscPrinter.DIRECTION.BACKWARD,
        reference: [0, 0],
        tear: BluetoothTscPrinter.TEAR.ON,
        sound: 1,
        text: [{
          text: 'I am a testing txt : ' + text,
          x: 20,
          y: 0,
        ...
        barcode: [{
          x: 120,
          y: 96,
          type: BluetoothTscPrinter.BARCODETYPE.ITF,
          height: 40,
          readable: 1,
          rotation: BluetoothTscPrinter.ROTATION.ROTATION_90,
          code: '1234567890'
        }]
      }
    );
  ...
  async function printLabel(text: String) {
    try {
      await BluetoothEscposPrinter.printerInit();
      await BluetoothEscposPrinter.verticalPrint(); 
      await BluetoothEscposPrinter.printerAlign(1); //center
      await BluetoothEscposPrinter.setWidth(BluetoothEscposPrinter.width80);
      await  BluetoothEscposPrinter.printText(text + "\r\n\r\n", {});

      await BluetoothEscposPrinter.printBarCode("82640000000256646032023011500053433100000001", BluetoothEscposPrinter.BARCODETYPE.ITF, 1, 80, 0, 2);
      await BluetoothEscposPrinter.printBarCode("8264000000025664603202301150", BluetoothEscposPrinter.BARCODETYPE.ITF, 2, 60, 0, 2);
      await BluetoothEscposPrinter.printBarCode("826400000002566460320230115", BluetoothEscposPrinter.BARCODETYPE.ITF, 1, 80, 0, 2);
      await BluetoothEscposPrinter.printQRCode("https://www.google.com/", 200, BluetoothEscposPrinter.ERROR_CORRECTION.M);
      await BluetoothEscposPrinter.printPic(base64JpgLogo, {width: 200, left: 40});
      await BluetoothEscposPrinter.printText("\r\n\r\n\r\n", {});


      await BluetoothEscposPrinter.printerLeftSpace(0);
      await BluetoothEscposPrinter.printColumn([
        BluetoothEscposPrinter.width80 / 8 / 3, 
        BluetoothEscposPrinter.width80 / 8 / 3 , 
        BluetoothEscposPrinter.width80 / 8 / 3],
        [BluetoothEscposPrinter.ALIGN.CENTER, BluetoothEscposPrinter.ALIGN.CENTER, BluetoothEscposPrinter.ALIGN.CENTER],
        ["Isso é um teste", 'Testando coluna 2', 'R$ 20,000,00'], {fonttype: 1});
      await  BluetoothEscposPrinter.printText("\r\n\r\n\r\n", {});
  ...
  React.useEffect(() => {
    if (GenericEscposPrint.isBluetoothEnabled()) {
      getDevices();
    } else {
      console.log("Bluetooth já esta ativado !");
    }
  }, []);

  return (
    <View style={styles.container}>
      <View style={{ marginVertical: 10, alignContent: 'center', alignItems: 'center', justifyContent: 'center' }}>
        <Button title={`imprimir na ${printer}`} onPress={() => { printLabel("Teste") }} color={'green'}></Button>
      </View>
      <View style={{ marginVertical: 10, alignContent: 'center', alignItems: 'center', justifyContent: 'center' }}>
        <Button title={`imprimir 2 na ${printer}`} onPress={() => { printTSC("Teste") }} color={'purple'}></Button>
      </View>
      <View style={{ marginVertical: 10, alignContent: 'center', alignItems: 'center', justifyContent: 'center' }}>
        <Button title={`imprimir 3 na ${printer}`} onPress={() => { print3() }} color={'orange'}></Button>
      </View>

      {devices.map((d, i) => {
        return <View key={i} style={styles.button}>
          <Button title={d.name + ' / ' + d.address} onPress={() => {
            connectDevice(d.address);
          }}></Button>
        </View>
      })}
    </View>
