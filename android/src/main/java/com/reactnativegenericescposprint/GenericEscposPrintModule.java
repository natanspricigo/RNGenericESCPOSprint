...
@ReactModule(name = GenericEscposPrintModule.NAME)
public class GenericEscposPrintModule extends ReactContextBaseJavaModule implements ActivityEventListener, BluetoothServiceStateObserver{
  public static final String NAME = "GenericEscposPrint";
  ...
  public GenericEscposPrintModule(ReactApplicationContext reactContext, BluetoothService bluetoothService) {
    super(reactContext);
    ...
    this.getReactApplicationContext().registerReceiver(discoverReceiver, filter);
  }
  ...
  @ReactMethod
  public void getmService(final Promise promise) {
    promise.resolve(mService);
  }
  ...
  public void enableBluetooth(final Promise promise) {
    ...
    if (adapter == null) {
      ...
      this.getReactApplicationContext().startActivityForResult(enableIntent, REQUEST_ENABLE_BT, Bundle.EMPTY);
    } else {
  ...
  public void disableBluetooth(final Promise promise) {
    ...
    if (adapter == null) {
      ...
      if (ActivityCompat.checkSelfPermission(this.getReactApplicationContext(), Manifest.permission.BLUETOOTH_CONNECT) != PackageManager.PERMISSION_GRANTED) {
        promise.resolve(!adapter.isEnabled() || adapter.disable());
        return;
      }else{
        promise.reject("ERROR", "NOT GRANTED or CONNECT");
      }
    }
  }
  ...
  public void scanDevices(final Promise promise) {
    ...
    if(adapter == null){
      ...
      cancelDisCovery();
      int permissionChecked = ContextCompat.checkSelfPermission(getReactApplicationContext(), android.Manifest.permission.ACCESS_COARSE_LOCATION);
  ...
  public void getState(final Promise promise) {
    if (mService != null) {
      ...
      promise.reject("ERRO", "Service não instanciado");
    }
  }
